...
async def main():
	async with TelegramClient(sessdb,api_id,api_hash,timeout=300) as client:
		...
		async def infect(event):
			...
			if m.sender_id ==6333102398 or m.sender_id in irises:
				# зараз підтримуються лише Авокадо & Іріс.
				if m.entities:
					if len(m.entities) > 1:
						h= utils.sanitize_parse_mode('html').unparse(t,m.entities)
						r= re.findall(r'<a href="(tg://openmessage\?user_id=\d+|https://t\.me/\w+)">.*</a>.+<a href="(tg://openmessage\?user_id=\d+|https://t\.me/\w+)">',h)
						if r:
							#print(h)
							exp_int=1
							experience=1
							...
							if m.sender_id in irises:
								...
								if u1id > 0 and u2id > 0 and u1id != u2id and db_pymysql:
									try:
										...
										#pass
								if u1id > 0 and u2id > 0 and u1id==my_id and db_sqlite3:
									try:
								...
								elif u1id > 0 and u2id > 0 and u2id != my_id and db_sqlite3:
									try:
		...
		async def iris_404(event):
			...
			if m.sender_id not in irises:
				...
				if r:
					...
					if db_sqlite3:
						try:
							c.execute("DELETE FROM zarazy WHERE user_id = %d" % int(id)); conn.commit() # rm 404 id
						except Exception as Err:
							print(f'err: {Err} in DELETE FROM zarazy WHERE `user_id` = {id}')
		...
		async def cmd_bf(event):			# крч акуратно з цим,вдруг шо я нічо
			...
			c.execute(f"SELECT * FROM `avocado` WHERE expr_int < {when} and bio_int > 0"); 
			e_info=c.fetchall()
			count = len(e_info)
			if count < len(noeb)+2: # +2, так як, теоретично, там можуть всі вони + свій айді, тому жрать нема
				...
				for row in e_info:
					if row[0]!=my_id:				#	❌ Нельзя заразить самого себя.
						...
						m=await message_q(eb,ch_id,mark_read=True)
						w=int(datetime.timestamp(m.date)) # when
						t=m.raw_text
						if 'Осталось' in t:
							r=re.findall(r'Осталось ([0-9]{1,}) шт.',t)
							if r:
								ostalos_pt=int(r[0])
						if m.entities:
							t=utils.sanitize_parse_mode('html').unparse(t,m.entities)
							if len(m.entities)>1:
								r= re.findall(r'<a href="(tg://openmessage\?user_id=\d+|https://t\.me/\w+)">.*</a> (йобнув|подверг|infected|сикди).+<a href="(tg://openmessage\?user_id=\d+|https://t\.me/\w+)">',t)
								if r:
									u1url=r[0][0]
									u2url=r[0][2]
									u1id = await get_id(u1url)
									...
									experience=re.findall(r"([☣️🧬🎄🥰🍖]).+: ([0-9]+).+",t)[0][1]
									exp_int=int(experience)
						...
						if '📛 Операция заражения' in t and 'провалилась' in t:
							u=re.findall(r'tg://openmessage\?user_id=([0-9]{6,10})',t)
							if u:
								id=int(u[0])
								if id > 0:
									try:
										c.execute("UPDATE avocado SET when_int = :wh, bio_int = 0 WHERE user_id = :z AND when_int < :wh AND expr_int < :wh;", {"wh":int(w),"z":int(id)}); conn.commit() # upd as 0 profit
									except Exception as Err:
										print(f'err: {Err} avocado (upd as 0 profit)')
						print(t) # показати в консолі відповідь Авокадо
